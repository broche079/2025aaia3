...
public:             ///vector 是C++ 的陣列, 伸縮自如, 但是2D比較難寫
    string tictactoe(vector<vector<int>>& moves) {
        ...
        for(vector<int>& move : moves){
            ...
            if(now==a[0][2] && now==a[1][1] && now==a[2][0]) winner = now; ///右下左上斜線
            if(now==1) now = 2; ///兩人要交換 1換2
            else now = 1;///或2換1
        }
        if(winner==1) return "A";
        else if(winner==2) return "B";
        else if(moves.size()==9) return "Draw";///平手的英文教Darw
        else return "Pending";///送出去 還發現第4種可能 叫Pending 還沒下完啊哈
    }
};
